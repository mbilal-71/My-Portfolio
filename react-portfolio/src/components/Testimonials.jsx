  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from('.test-hdr', {
        ...
        scrollTrigger: { trigger: '.test-hdr', start: 'top 87%' },
      });
      gsap.from('.test-card', {
        opacity: 0, y: 40, duration: 0.65, stagger: 0.12, ease: 'power3.out',
        scrollTrigger: { trigger: '.test-grid', start: 'top 83%' },
      });
    }, ref);
    return () => ctx.revert();
  }, []);
      ...
      <div className="section-container">
        ...
        <div className="test-grid grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          {SAMPLE_TESTIMONIALS.map((t) => (
            <div
              key={t.id}
              className="test-card card flex flex-col gap-4 relative overflow-hidden"
            >
              {/* Sample label */}
              {t.isSample && (
                <div
                  className="absolute top-3 right-3 text-[0.55rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full"
                  style={{
              ...
              <div className="flex flex-wrap gap-1.5">
                {t.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2.5 py-0.5 rounded-full text-[0.6rem] font-semibold"
                    style={{
              ...
              <div className="flex items-center gap-2 pt-2" style={{ borderTop: '1px solid var(--border-subtle)' }}>
                ...
                <div>
                  ...
                  <p className="text-[0.6rem]" style={{ color: 'var(--text-muted)' }}>{t.source}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {/* CTA card — 3rd column */}
          <div
            className="test-card flex flex-col items-center justify-center gap-5 rounded-xl py-10 px-6 text-center"
            style={{
              background: '#30280F',
